...
export class ToolboxSearchCategory extends Blockly.ToolboxCategory {
  ...
  protected override createDom_(): HTMLDivElement {
    ...
    this.searchField.placeholder = this.name_;
    this.workspace_.RTL
  ...
  private matchBlocks() {
    ...
    this.flyoutItems_ = query
      ? this.blockSearcher.search(query).map((blockType) => {
          return {
            kind: 'block',
            type: blockType,
          };
    ...
    if (!this.flyoutItems_.length) {
      this.flyoutItems_.push({
        ...
        text:
          query.length === 0
            ? Blockly.Msg['TOOLBOX_SEARCH_TOO_SHORT']
            : Blockly.Msg['TOOLBOX_SEARCH_NO_MATCHES'],
      });
    }
